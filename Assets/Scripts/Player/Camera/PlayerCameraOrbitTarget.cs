...
public class PlayerCameraOrbitTarget : MonoBehaviour
{
    ...
    private void UpdateRotation()
    {
        ...
        yaw += lookInput.x * config.MouseSensitivity;
        pitch += lookInput.y * config.MouseSensitivity*-1;
    ...
    private void UpdateDistanceByPitch()
    {
        ...
        float topViewRate = Mathf.InverseLerp(config.BeginTopViewPitch, config.MaxPitch, pitch);
        //拿到比例之后算出具体的当前值是多少
        float targetDistance = Mathf.Lerp(config.NormalDistance, config.TopViewDistance, topViewRate);
        //当前的距离由当前值到目标值并由设置的速度移动
        currentDistance = Mathf.Lerp(currentDistance, targetDistance, config.DistanceSmoothSpeed * Time.deltaTime);
        //设置位置即可
